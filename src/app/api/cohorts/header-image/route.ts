  ...
  try {
    ...
    if (!mimeType) {
      ...
      if (!ext || !ALLOWED_IMAGE_EXTENSIONS.has(ext)) {
        return jsonError("Header image must be JPEG, PNG, or WebP.");
      }
    ...
    const path = `cohort-hub/${userResult.user.id}/${crypto.randomUUID()}.${extension}`;
    const admin = supabaseAdminClient();
    ...
    if (uploadError) {
      console.error("[cohort-hub] header image upload error:", uploadError.message);
    ...
    console.error("[cohort-hub] header image route error:", err);
    return jsonError("Failed to upload header image.", 500);
  }
}
